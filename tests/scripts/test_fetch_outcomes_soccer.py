"""
End-to-end tests for scripts/fetch_outcomes_soccer.py — soccer game-outcome grading.

Hits a real SQLite DB but stubs the ESPN soccer scoreboard via the script's
``scoreboard_fetcher`` injection point (signature: (iso_date, league)) — no network.

Covers:
- Home-win grading
- Draw grading (3-way result → result == "draw")
...
class TestFetchOutcomesSoccer:
    ...
    def test_league_filter_skips_other_competitions(self):
        ...
        # Provide a La Liga final, but only ask for EPL → nothing should attach.
