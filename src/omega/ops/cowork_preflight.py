...
def repair_source_integrity(repo_root: Path) -> list[str]:
    ...
    for py_file in sorted(repo_root.rglob("*.py")):
        ...
        try:
            ...
            failures.append(
                f"Source corrupt (AST parse failed) {py_file}:{e.lineno} â€” {e.msg}. "
...
def verify_against_git(
    ...
    through the mount cache (Write-tool writes do not â€” see
    ...
    if repair:
        ...
        if force_repair:
            ...
            # gets restored. Test-tier corruption is restored too â€” that's
            # still a real corruption event we want to heal â€” but a *clean*
    ...
    # Non-critical core divergences are surfaced as warnings only â€” they are
