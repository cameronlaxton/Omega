"""
scripts/capture_early_lines.py — capture low-liquidity early lines (Phase 7).
...
table ONLY — never ``closing_lines``. The canonical CLV computation reads only
...
Usage:
    OMEGA_ODDS_API_KEY=xxxx python scripts/capture_early_lines.py --leagues WNBA
...
def _capture_league(
    ...
    if liquidity_profile != "low":
        return 0, [
            ...
            "for low-liquidity leagues only) — skipped"
        ]

    if sport_key_for(league) is None:
        return 0, [f"{league}: no sport_key mapping in odds_api.py — skipped"]
