...
Usage:
    python scripts/run_baseline_sweep.py [--league NBA] [--json data/sweep_out.json]

Data sourcing (no network calls):
  1. TraceStore.query_traces(has_outcome=True) -- uses real graded data if present
  2. Falls back to scripts/fixtures/baseline_games.json if zero graded traces found
...
def _print_diagnostics(games: list[dict]) -> None:
    ...
    for game in games:
        try:
            ...
            print(f"  {game.get('home_team', '?')} — error: {exc}")
