...
def test_apply_calibration_audited_sport_mapping(monkeypatch):
    ...
    val, audit = apply_calibration_audited(0.5, league="NFL", context_hints={"thursday": True})
    assert audit["resolved_slice"] == "short_week"
    assert audit["profile_id"] == "prof_NFL_short_week"
    
    # Soccer cup_match mapping
    val, audit = apply_calibration_audited(0.5, league="EPL", context_hints={"knockout": True})
    assert audit["resolved_slice"] == "cup_match"
    assert audit["profile_id"] == "prof_EPL_cup_match"
    
    # NBA back to back
    val, audit = apply_calibration_audited(0.5, league="NBA", context_hints={"rest_days": 0})
