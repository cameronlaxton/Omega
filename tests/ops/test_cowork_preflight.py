...
def _init_repo(tmp_path: Path) -> Path:
    ...
    service = repo / "src" / "omega" / "core" / "contracts" / "service.py"
...
def test_repair_from_git_proceeds_when_only_tests_are_dirty(tmp_path):
    ...
    assert failures == []
    assert service.read_text(encoding="utf-8") == "VALUE = 1\n"
    # The dirty test edit is preserved â€” we deliberately did not touch it.
...
def test_split_tiers_separates_tests_from_core():
    core, test = cowork_preflight._split_tiers(
        [
            "src/omega/core/contracts/service.py",
            "tests/core/test_engine.py",
            "src/omega/ops/run_analyze.py",
            "tests/ops/test_cowork_preflight.py",
            "OMEGA_COWORK.md",
        ]
    )
    assert core == [
