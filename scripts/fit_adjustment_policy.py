"""
scripts/fit_adjustment_policy.py — fit a CANDIDATE engine adjustment policy.

Closes the structured-reasoning loop: it reads retrospective signal-performance
aggregates (produced by scripts/score_evidence_signals.py) and derives a
...
script never promotes and never changes live behavior — promotion is a separate,
gated, operator-driven step (scripts/promote_adjustment_policy.py).
...
Usage:
    python scripts/fit_adjustment_policy.py
    python scripts/fit_adjustment_policy.py --league NBA --min-samples 30
    python scripts/fit_adjustment_policy.py --mode live --dry-run

Exit codes:
    0 — candidate registered (or dry-run completed)
    1 — fatal error or no signal-performance data available
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not perf_rows:
        logger.error(
            "No signal-performance data. Run scripts/score_evidence_signals.py first."
        )
        return 1
    ...
    for signal_type, (total_n, accuracy) in sorted(aggregates.items()):
        if total_n < args.min_samples:
            logger.info(
                "  skip %-22s n=%-3d (< %d) — keeps full trust",
    ...
    if args.dry_run:
        logger.info(
            "DRY-RUN — candidate %s (version %d, mode=%s, %d signals fitted).",
        ...
        logger.info(
            ...
            "Review and promote with scripts/promote_adjustment_policy.py.",
