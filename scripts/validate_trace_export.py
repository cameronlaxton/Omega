"""
scripts/validate_trace_export.py — validate exported trace files before ingest.
...
re-export — do not re-run analyze().**
...
Usage:
    python scripts/validate_trace_export.py inbox/traces
    python scripts/validate_trace_export.py inbox/traces/foo.json --lenient
    python scripts/validate_trace_export.py inbox/traces --json

Exit codes:
    0 — all validated files pass (no errors)
    1 — at least one file has an error, or a path was unreadable
"""
