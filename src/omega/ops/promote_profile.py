"""
omega-promote-profile â€” promote a CANDIDATE calibration profile to PRODUCTION.
...
Gates:
    1. SAMPLE_SIZE      â€” candidate.sample_size >= --min-samples (default 100)
    2. BRIER_IMPROVES   â€” candidate.brier_score is at least --brier-improvement
                          (default 0.01) lower than incumbent.brier_score
    3. LOG_LOSS_NO_REG  â€” candidate.log_loss is not worse than incumbent.log_loss
                          by more than --log-loss-tol (default 0.005)
    4. BACKTEST_PARITY  â€” backtest-replay ROI parity within Â±0.5%
                          [REQUIRES DATA] (skipped until graded bets accumulate; see notes)
    5. CLV_NON_REG      â€” mean CLV cents does not regress by more than 0.5 cents
...
`omega-fit-calibration`. Comparison metrics come from each profile's stored
...
Usage:
    ...
    omega-promote-profile --candidate-id <id> --force
    omega-promote-profile --list-candidates --league NBA

Exit codes:
    0 â€” promotion succeeded (or list completed)
    1 â€” any gate failed and --force was not supplied
    2 â€” fatal error (unknown candidate, etc.)
"""
...
def _evaluate_gates(
    ...
    if incumbent is None:
        results.append(("BRIER_IMPROVES", True, "no incumbent â€” auto-pass"))
        ...
        if inc_brier is None:
            results.append(("BRIER_IMPROVES", True, "incumbent has no brier_score â€” auto-pass"))
        else:
    ...
    if incumbent is None:
        results.append(("LOG_LOSS_NO_REG", True, "no incumbent â€” auto-pass"))
        ...
        if inc_log is None:
            results.append(("LOG_LOSS_NO_REG", True, "incumbent has no log_loss â€” auto-pass"))
        else:
...
def main() -> int:
    ...
    gates.append(
        (
            ...
            else "no automated check â€” confirm with --confirm-backtest-parity after backtest review",
        )
    )
    gates.append(
        (
            ...
            else "no automated check â€” confirm with --confirm-clv-non-regression after CLV review",
        )
    )
