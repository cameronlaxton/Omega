"""
scripts/backfill_input_snapshot.py — one-time provenance-first historical fix.
...
Usage:
    python scripts/backfill_input_snapshot.py          # dry-run (default)
    python scripts/backfill_input_snapshot.py --apply  # commit changes

Exit codes:
    0 — completed (even if some rows were unrecoverable — check output)
    1 — fatal error
"""
...
def _patch_trace(
    ...
    # Update trace_quality — prefer existing trace_quality, fall back to quality_gate.
...
def run(db_path: str, apply: bool) -> int:
    ...
    for row in rows:
        ...
        if not recovered and not unrecoverable:
            # All fields were already present — shouldn't happen given the SQL filter.
            n_noop += 1
            logger.debug("trace %s — no-op (all fields already present)", trace_id)
            continue
        ...
        if recovered:
            n_patched += 1
            logger.info(
                "trace %s — recovered %s; unrecoverable %s",
                trace_id, recovered, unrecoverable or "none",
            )
        else:
            n_unrecoverable += 1
            logger.warning(
                "trace %s — no recoverable fields (result also missing); tagged identity_status=missing",
                trace_id,
            )
    ...
    if not apply:
        logger.info("Dry-run mode — no changes committed. Pass --apply to commit.")
        conn.close()
        return 0
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "One-time backfill: copy identity fields from full_trace.result → "
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="SQLite database path (default: repo root omega_traces.db)",
    )
    ...
    if args.db:
        ...
        db_path = str(_REPO_ROOT / "omega_traces.db")
