"""
scripts/backfill_trace_quality.py — repopulate calibration-eligibility metadata
...
Background:
    ...
    calibration-eligibility gate in scripts/report_calibration.py /
    scripts/fit_calibration.py reads
...
Policy reuse (no drift):
    ...
    uses — `omega.core.contracts.service.derive_calibration_eligibility` and
...
Provenance:
    ...
    from natively-engine-computed eligibility. This is a DATA migration only —
...
Usage:
    python scripts/backfill_trace_quality.py --dry-run          # report only
    python scripts/backfill_trace_quality.py                    # apply
    python scripts/backfill_trace_quality.py --league NBA       # filter
    python scripts/backfill_trace_quality.py --db <path> -v
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--db", default=None, help="DB path (default: repo-root omega_traces.db)")
