"""
omega.ops.fetch_outcomes_nba â€” attach NBA final scores from ESPN to ungraded traces.

Workflow:
    1. Determine the date range (default: yesterday in ET).
    2. For each date, fetch the ESPN scoreboard once.
    3. Query the trace store for ungraded NBA traces in that range.
...
Usage:
    ...
    omega-fetch-outcomes-nba --since 2026-05-10 --until 2026-05-14
    omega-fetch-outcomes-nba --since yesterday
    omega-fetch-outcomes-nba --dry-run
...
from omega.integrations.espn_nba import FinalGame, canonical_team, fetch_scoreboard  # noqa: E402
from omega.trace.store import TraceStore, log_effective_db  # noqa: E402

logger = logging.getLogger("fetch_outcomes_nba")
...
def main(
    ...
    *,
    ...
    leaves it None and the live ESPN NBA fetcher is used.
    """
    ...
    for d in _iter_dates(start, end):
        ...
        # Pull ungraded NBA traces in this date window
        ...
        for trace in traces:
            ...
            try:
                ...
                # is not currently enforced â€” multiple outcomes per trace would
