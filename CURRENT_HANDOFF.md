...
| `omega-resolve-odds` | BetMGM-first current odds resolver |
...
**Problem:** `TraceStore` detects that `var/omega_traces.db` is on a FUSE/network mount and redirects all SQLite writes to a local sandbox path (`~/.omega/runtime/omega_traces.db`). This is correct behavior — SQLite on FUSE is unreliable (BUG-FUSE-2). But it means every scheduled task run writes to a path that is destroyed when the Linux sandbox session expires. The Windows-side `var/omega_traces.db` is never updated.
