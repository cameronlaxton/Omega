"""
scripts/db_status.py — read-only TraceStore DB doctor.
...
— WITHOUT instantiating TraceStore, so it still works when the normal redirect
...
Usage:
    ...
    python scripts/db_status.py --json
    python scripts/db_status.py --db /path/to/omega_traces.db
    python scripts/db_status.py --seed        # explicit: populate absent runtime DB

Exit codes:
    0 — report produced (and seed succeeded if requested)
    1 — effective DB exists but fails integrity_check, or --seed preconditions failed
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--db", default=None, help="Requested DB path (default: repo-root)")
