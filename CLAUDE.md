...
**Hard rule (supersedes all prior fallback wording in OMEGA_RUN_RECIPE.md and OMEGA_HANDBOOK.md):** The LLM is forbidden from generating any of the above values via text. They must come from Python execution through the local MCP server or `omega.core.contracts.service.analyze`. There is no "estimated", "rough", "ballpark", `[CLAUDE-ESTIMATED]`, or "estimated lean" mode for these fields. If the engine is unavailable, the response is qualitative-only: matchup narrative, news, recent form, listed sportsbook lines from a cited source — never a Bet Card with placeholder numbers.
