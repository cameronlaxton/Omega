"""
scripts/backfill_closing_lines.py — backfill missed closing-line windows from the
...
Usage:
    ...
    python scripts/backfill_closing_lines.py --league NBA --since 2026-05-10 --until 2026-05-14 --close-time 01:00:00Z

    # Backfill one specific trace at an exact timestamp
    python scripts/backfill_closing_lines.py --trace-id sandbox-abc123 --at 2026-05-14T21:30:00Z

    # Dry run (no DB writes, still hits the API)
    python scripts/backfill_closing_lines.py --league NBA --since 2026-05-10 --dry-run
...
def _close_timestamp_for_bet(bet: dict, explicit_at: str | None, close_offset_hours: int) -> str:
    ...
    the line closed (typically 4–8 h after the decision is logged).
    """
    if explicit_at:
        return explicit_at
