...
sibling .error.txt — no partial writes.
...
Workflow:
    ...
    2. For each file: parse → validate → apply each `lines` entry via TraceStore →
       move file to processed/.
    3. On any error: move file to failed/ with .error.txt sidecar.

Usage:
    python scripts/ingest_closing_lines.py
    python scripts/ingest_closing_lines.py --inbox <path> --db <path>
    python scripts/ingest_closing_lines.py --dry-run
...
def main() -> int:
    ...
    parser.add_argument(
        "--db", type=str, default=None, help="SQLite path (default: repo-root omega_traces.db)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
