...
def test_typed_predictions_recommendations_round_trip(tmp_path):
    from omega.core.models import ExecutionTrace, GamePredictions, Recommendation
    
    trace = ExecutionTrace(
        prompt="Test prompt",
        predictions=GamePredictions(home_win_prob=0.55, over_prob=0.60),
        recommendations=[
            Recommendation(side="home", edge_pct=3.5, recommendation="over", units=1.0)
        ],
    )
    
    # 1. Verify attribute access
    assert trace.predictions is not None
    assert trace.predictions.home_win_prob == 0.55
    assert trace.predictions.over_prob == 0.60
    assert trace.recommendations[0].side == "home"
    assert trace.recommendations[0].recommendation == "over"
    
    # 2. Verify dictionary/mapping compatible get and __getitem__ helpers
    assert trace.predictions.get("home_win_prob") == 0.55
    assert trace.predictions["home_win_prob"] == 0.55
    assert trace.recommendations[0].get("edge_pct") == 3.5
    assert trace.recommendations[0]["edge_pct"] == 3.5
    
    # 3. Verify null safety
    empty_trace = ExecutionTrace(prompt="Empty")
    assert empty_trace.predictions is None
    
    # 4. JSON serialization round-trip parity
    dumped = trace.model_dump(mode="json")
    assert dumped["predictions"]["home_win_prob"] == 0.55
    assert dumped["recommendations"][0]["side"] == "home"
    
    reloaded = ExecutionTrace.model_validate(dumped)
    assert reloaded.predictions is not None
    assert reloaded.predictions.home_win_prob == 0.55
    assert reloaded.recommendations[0].side == "home"
