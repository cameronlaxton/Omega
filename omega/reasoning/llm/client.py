"""
LLM client for the agent pipeline.

Uses an abstract ``LLMProvider`` base class so additional providers can
be added later.  The ``LLMClient`` facade picks the right provider based
on ``AgentConfig.llm_provider`` and delegates all calls.
"""
