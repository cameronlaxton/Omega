"""
omega-run-action-plan â€” dispatch the LLM's action-plan JSON to deterministic scripts.

The LLM, at session start, emits an action plan per system_prompt.txt Â§13. This
...
Allowlist:
    ...
    type=fetch_outcomes
        args.leagues: list[str] (default ["nba", "wnba", "mlb", "soccer", "nhl", "tennis", "props"])
...
Exit codes:
    0 â€” all actions dispatched successfully (or dry-run completed)
    1 â€” at least one action failed
    2 â€” fatal validation error (file missing, unknown action type, bad args)
"""
...
def _module_cmd(module: str, *args: str) -> list[str]:
    return [sys.executable, "-m", f"omega.ops.{module}", *args]
...
def _validate_promote_profile(args: dict[str, Any]) -> list[str]:
    ...
    cmd = _module_cmd("promote_profile", "--candidate-id", candidate_id)
    if auto:
        cmd.append("--auto")
    return cmd
...
def _validate_report_calibration(args: dict[str, Any]) -> list[str]:
    ...
    cmd = _module_cmd("report_calibration", "--league", league, "--window-days", str(window_days))
    return cmd


def _validate_fetch_outcomes(args: dict[str, Any]) -> list[str]:
    _reject_unknown_args("fetch_outcomes", args, {"leagues", "since", "until"})
    from omega.ops.fetch_outcomes_all import DEFAULT_LEAGUES, VALID_LEAGUES  # noqa: PLC0415

    leagues = args.get("leagues", list(DEFAULT_LEAGUES))
    ...
    unknown = set(leagues) - VALID_LEAGUES
    if unknown:
    ...
    cmd = _module_cmd("fetch_outcomes_all", "--leagues", *leagues)
    if since:
        cmd += ["--since", since]
    if until:
        cmd += ["--until", until]
...
def _validate_fetch_closing_lines(args: dict[str, Any]) -> list[str]:
    ...
    cmd = _module_cmd("fetch_closing_lines")
    if league:
        cmd += ["--league", league]
    if verbose:
        cmd.append("--verbose")
...
def _validate_score_evidence_signals(args: dict[str, Any]) -> list[str]:
    ...
    cmd = _module_cmd("score_evidence_signals")
    if league:
        cmd += ["--league", league.upper()]
