...
- `omega_fetch_outcomes`: batch-gathers outcomes across leagues (wraps `omega.ops.fetch_outcomes_all`). Defaults to all leagues, including tennis; pass `leagues` without `"soccer"` to exclude future-dated fixtures, or pass `"atp"` / `"wta"` to route a single tennis tour through the tennis outcome fetcher. `dry_run=True` reports what would run. Returns per-league `ok` / `failures` / `results`; the MCP envelope can be `status="success"` while `ok=false` for partial league failure, so callers must read the payload status rather than the envelope alone.
