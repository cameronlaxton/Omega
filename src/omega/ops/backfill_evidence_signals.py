"""
omega.ops.backfill_evidence_signals â€” re-explode evidence_signals from frozen
...
pre-decision ``input_snapshot``, provenance is ``original`` â€” there is no separate
...
results are NEVER read â€” they cannot manufacture evidence. A trace whose snapshot
evidence is genuinely empty is left untouched and counted as unrecoverable; no
fake signals are invented.

Usage:
    omega-backfill-evidence-signals                 # dry-run (default)
    omega-backfill-evidence-signals --dry-run --verbose
    omega-backfill-evidence-signals --apply         # writes
    omega-backfill-evidence-signals --trace-id sandbox-...

Exit codes:
    0 â€” scan completed (dry-run or apply)
...
def _mark_evidence_present(store: TraceStore, trace_id: str, trace: dict) -> None:
    ...
    touched. Provenance is 'original' â€” the data came from the trace's own frozen
