...
def _fake_scoreboard_factory(games_by_league_date: dict[tuple, list[FinalGame]]):
    """Return a (league, date) ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ games callable using the supplied map."""
...
def _wnba_payload(player: str, team: str, pts: float, reb: float, ast: float) -> dict[str, Any]:
    """WNBA box score ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â identical ESPN basketball shape as NBA."""
    return {
        "boxscore": {
            "players": [
                {
...
class TestFetchOutcomesPropsSkips:
    ...
    def test_player_name_normalization_matches_accents(self):
        """Trace says 'Luka Doncic', ESPN says 'Luka Don\u010di\u0107' and still grades."""
        db = _tmp_store_path()
        store = TraceStore(db_path=db)
        store.persist(
            _make_prop_trace(
