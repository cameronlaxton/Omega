"""
omega-fetch-outcomes-props â€” attach ESPN box-score stats to ungraded prop traces.
...
This script is the player_prop outcome-attachment adapter â€” it exists alongside
...
Workflow:
    ...
    3. Group traces by (game_date, home_team, away_team) and fetch the ESPN
       scoreboard once per league/date pair to resolve game pair â†’ event_id.
    ...
    6. Report unmatched traces (missing game fields, unknown player, unmapped
       ...
       omega-backfill-outcomes-manual.
...
Exit codes:
    ...
    1 â€” fatal error (bad args, ESPN unreachable on first fetch)
"""
...
def main(
    ...
    *,
    ...
    # Dedup sets â€” the same trace_id can appear across multiple date windows
    ...
    for league in leagues:
        for d in _iter_dates(start, end):
            ...
            # Idempotent merge â€” skip ids already in `traces` to avoid
            # double-processing.
