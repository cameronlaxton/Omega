"""
omega-promote-adjustment-policy â€” promote a CANDIDATE adjustment policy.

Promotion makes a fitted set of evidence coefficients the production policy.
It is gated and operator-driven â€” never automatic â€” because promoting (and
...
Gates:
    ...
    2. BACKTEST_IMPROVES â€” shadow-recorded adjustments improve Brier/ECE on a
                           held-out window [REQUIRES DATA] â€” there is no
...
Usage:
    omega-promote-adjustment-policy --list-candidates
    omega-promote-adjustment-policy --candidate-id adj_v2_abc123
    omega-promote-adjustment-policy --candidate-id <id> --auto --confirm-backtest
    omega-promote-adjustment-policy --candidate-id <id> --auto \\
        --confirm-backtest --go-live

Exit codes:
    0 â€” promotion succeeded (or list completed)
    1 â€” a gate failed and --force was not supplied
    2 â€” fatal error (unknown candidate, wrong status)
"""
...
def _evaluate_gates(
    ...
    results.append(
        (
            ...
            if confirm_backtest
            else "no automated check â€” confirm with --confirm-backtest after "
