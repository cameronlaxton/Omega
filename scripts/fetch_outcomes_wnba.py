"""
scripts/fetch_outcomes_wnba.py — attach WNBA final scores from ESPN to ungraded traces.

Game-plane twin of fetch_outcomes_nba.py for WNBA. Player props are graded
separately by scripts/fetch_outcomes_props.py (which already supports WNBA);
this script owns WNBA game (moneyline/spread) outcomes only and never touches
prop traces.

Workflow:
    1. Determine the date range (default: yesterday in UTC).
    2. For each date, fetch the ESPN WNBA scoreboard once.
    3. Query the trace store for ungraded WNBA game traces in that range
       (game date and the prior day, for decisions made the day before).
    4. Match each ungraded trace to a FinalGame by canonical (home, away).
    5. Attach the outcome via store.attach_outcome(..., source="api:espn").
    6. Report unmatched traces so unmapped aliases can be added to
       omega/integrations/espn_wnba.py::WNBA_TEAMS.

Usage:
    python scripts/fetch_outcomes_wnba.py
    python scripts/fetch_outcomes_wnba.py --since 2026-05-17 --until 2026-05-17
    python scripts/fetch_outcomes_wnba.py --dry-run

Exit codes:
    0 — completed (may have unmatched; see log)
    1 — fatal error (bad args, ESPN unreachable)
"""
...
from omega.integrations.espn_wnba import (  # noqa: E402
    FinalGame,
    canonical_team,
    fetch_scoreboard,
)
...
logger = logging.getLogger("fetch_outcomes_wnba")
...
def main(
    ...
    *,
    scoreboard_fetcher: Callable[[str], Any] | None = None,
) -> int:
    """CLI entry point.
    ...
    leaves it None and the live ESPN WNBA fetcher is used.
    """
    parser = argparse.ArgumentParser(description="Attach ESPN final scores to WNBA game traces")
    parser.add_argument(
    ...
    for d in _iter_dates(start, end):
        logger.info("Fetching ESPN WNBA scoreboard for %s", d)
        try:
            games = sb_fetch(d.isoformat())
        except Exception as exc:  # noqa: BLE001
            logger.error("ESPN WNBA fetch failed for %s: %s", d, exc)
            return 1
        ...
        # Ungraded WNBA traces in this date window plus the prior day.
    ...
    if unmatched:
        logger.warning("Unmatched traces (consider extending WNBA_TEAMS aliases):")
