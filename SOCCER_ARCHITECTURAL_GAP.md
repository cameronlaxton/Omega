...
| 4 — Draw calibration plane | ✅ Closed | `market` dimension on `CalibrationProfile` + registry selection (game-profile fallback); `extract_draw_pairs`; `fit_calibration.py --plane draw` (`omega/core/calibration/*`, `omega-fit-calibration`) |
